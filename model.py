...
class RAM(object):
    ...
    def build_model(self):
        with tf.name_scope('inputs'):
            ...
            #self.istest=tf.placeholder(tf.int32, None) #My added
        ...
        with tf.name_scope('dynamic_rnn'):
            ...
            for h in range(self.n_hop):
                ...
                def body(i, newe, score):
                    ...
                    #print (self.istest)
                    #g= tf.cond(tf.equal(self.istest,0), lambda: tf.matmul(weights['attention'][h], tf.transpose(tf.concat([a, b, c], 1), perm=[1, 0])) + biases['attention'][h], lambda: tf.cast(tf.Variable(np.full((1,self.max_sentence_len), 1/self.max_sentence_len, dtype=None, order='C'),tf.float32),tf.float32) )
                    #g= tf.cond(tf.equal(self.istest,0), lambda: tf.matmul(weights['attention'][h], tf.transpose(tf.concat([a, b, c], 1), perm=[1, 0])) + biases['attention'][h], lambda: tf.cast(tf.Variable(np.random.normal(loc=0.0, scale=1.0, size=(1, self.max_sentence_len)),tf.float32),tf.float32) )
                    #g= tf.cond(tf.equal(self.istest,0), lambda: tf.matmul(weights['attention'][h], tf.transpose(tf.concat([a, b, c], 1), perm=[1, 0])) + biases['attention'][h], lambda: tf.cast(tf.Variable(np.random.uniform(low=0.0, high=1.0, size=(1,self.max_sentence_len)),tf.float32),tf.float32) ) 
                    g = tf.matmul(weights['attention'][h], tf.transpose(tf.concat([a, b, c], 1), perm=[1, 0])) + biases['attention'][h]
                    #g=tf.cast(tf.Variable(np.full((1,self.max_sentence_len), 1/self.max_sentence_len, dtype=None, order='C'),tf.float32),tf.float32) 

                    #t1=np.random.uniform(low=0.0, high=1.0, size=(1,self.max_sentence_len))
                    #t1=np.random.normal(loc=0.0, scale=1.0, size=(1, self.max_sentence_len))#
                    #g = tf.Variable(t1,tf.float32)
    ...
    def run(self, train_data1,validation_data1,train_data2,validation_data2, train_data3,validation_data3, train_data4,validation_data4, train_data,test_data):
        ...
        for i in range(self.n_epoch):
            train_loss1, train_acc1 = self.train(train_data1)
            validation_loss1, validation_acc1 = self.test(validation_data1)
            #test_loss, test_acc = self.test(test_data)
            accuracy_list[i]=accuracy_list[i] + validation_acc1
            if validation_acc1 > max_acc1:
                max_acc1 = validation_acc1
                step1 = i
                #saver.save(self.sess, 'models/model_iter' + str(self.timestamp), global_step=i)
            print('epoch %s: train-loss=%.6f; train-acc=%.6f; validation-loss=%.6f; validation-acc=%.6f;' % (str(i), train_loss1, train_acc1, validation_loss1, validation_acc1))
        print('The max accuracy of validation results is %s of step %s' % (max_acc1, step1))

        self.sess.run(tf.global_variables_initializer()) 
        max_acc2, step2 = 0., -1
        for i in range(self.n_epoch):
            train_loss2, train_acc2 = self.train(train_data2)
            validation_loss2, validation_acc2 = self.test(validation_data2)
            #test_loss, test_acc = self.test(test_data)
            accuracy_list[i]=accuracy_list[i] + validation_acc2
            if validation_acc2 > max_acc2:
                max_acc2 = validation_acc2
                step2 = i
                #saver.save(self.sess, 'models/model_iter' + str(self.timestamp), global_step=i)
            print('epoch %s: train-loss=%.6f; train-acc=%.6f; validation-loss=%.6f; validation-acc=%.6f;' % (str(i), train_loss2, train_acc2, validation_loss2, validation_acc2))
        print('The max accuracy of validation results is %s of step %s' % (max_acc2, step2))


        self.sess.run(tf.global_variables_initializer())
        max_acc3, step3 = 0., -1
        for i in range(self.n_epoch):
            train_loss3, train_acc3 = self.train(train_data3)
            validation_loss3, validation_acc3 = self.test(validation_data3)
            #test_loss, test_acc = self.test(test_data)
            accuracy_list[i]=accuracy_list[i] + validation_acc3
            if validation_acc3 > max_acc3:
                max_acc3 = validation_acc3
                step3 = i
                #saver.save(self.sess, 'models/model_iter' + str(self.timestamp), global_step=i)
            print('epoch %s: train-loss=%.6f; train-acc=%.6f; validation-loss=%.6f; validation-acc=%.6f;' % (str(i), train_loss3, train_acc3, validation_loss3, validation_acc3))
        print('The max accuracy of validation results is %s of step %s' % (max_acc3, step3))

        self.sess.run(tf.global_variables_initializer())
        max_acc4, step4 = 0., -1
        for i in range(self.n_epoch):
            train_loss4, train_acc4 = self.train(train_data4)
            validation_loss4, validation_acc4 = self.test(validation_data4)
            #test_loss, test_acc = self.test(test_data)
            accuracy_list[i]=accuracy_list[i] + validation_acc4
            if validation_acc4 > max_acc4:
                max_acc4 = validation_acc4
                step4 = i
                #saver.save(self.sess, 'models/model_iter' + str(self.timestamp), global_step=i)
            print('epoch %s: train-loss=%.6f; train-acc=%.6f; validation-loss=%.6f; validation-acc=%.6f;' % (str(i), train_loss4, train_acc4, validation_loss4, validation_acc4))
        print('The max accuracy of validation results is %s of step %s' % (max_acc4, step4))
         
        index, value = max(enumerate(accuracy_list), key=operator.itemgetter(1)) 
        print(accuracy_list)
        self.sess.run(tf.global_variables_initializer())
        for i in range(self.n_epoch):
         ...
         if i==index:
          print('epoch %s:  Max  value correspond to validation:: test-loss=%.6f; test-acc=%.6f;' % (str(i), test_loss, test_acc))
        print('Analyzing ...')
