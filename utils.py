...
def read_train_data(fname, word2id, max_sentence_len, max_aspect_len, save_fname, pre_processed,train,mod4):
                ...
                if len(sptoks.text.strip()) != 0:
                    ...
                    for asp_terms in sentence.iter('aspectTerms'):
                        for asp_term in asp_terms.findall('aspectTerm'):
                            ...
                            count = count + 1
                            if count % 4 == mod4 and train==1:
                             continue
                            if count % 4 != mod4 and train==0:
                             continue
...
def read_test_data(fname, word2id, max_sentence_len, max_aspect_len, save_fname, pre_processed):
    sentences, aspects, sentence_lens, sentence_locs, labels = [], [], [], [], []
    if pre_processed:
        if not os.path.isfile(save_fname):
            raise IOError(ENOENT, 'Not a file', save_fname)
